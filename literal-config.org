...
** list of packages
This is the list of packages to install. Remember, installing packages from melpa does not mean loading the into  Emacs.
...
#+begin_src emacs-lisp
  (use-package jacek-verse
    ...
    :init
    (melpa-install-packages '(ido-completing-read+))
    :load-path "modules/"
    :bind ("<f5>" . verse-link)
    )
#+end_src
